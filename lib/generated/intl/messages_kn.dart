// DO NOT EDIT. This is code generated via package:intl/generate_localized.dart
// This is a library that provides messages for a kn locale. All the
// messages from the main program should be duplicated here with the same
// function name.

// Ignore issues from commonly used lints in this file.
// ignore_for_file:unnecessary_brace_in_string_interps, unnecessary_new
// ignore_for_file:prefer_single_quotes,comment_references, directives_ordering
// ignore_for_file:annotate_overrides,prefer_generic_function_type_aliases
// ignore_for_file:unused_import, file_names, avoid_escaping_inner_quotes
// ignore_for_file:unnecessary_string_interpolations, unnecessary_string_escapes

import 'package:intl/intl.dart';
import 'package:intl/message_lookup_by_library.dart';

final messages = new MessageLookup();

typedef String MessageIfAbsent(String messageStr, List<dynamic> args);

class MessageLookup extends MessageLookupByLibrary {
  String get localeName => 'kn';

  final messages = _notInlinedMessages(_notInlinedMessages);
  static Map<String, Function> _notInlinedMessages(_) => <String, Function>{
    "account": MessageLookupByLibrary.simpleMessage("ಖಾತೆ"),
    "add_additional_details": MessageLookupByLibrary.simpleMessage(
      "ಇನ್ನಷ್ಟು ಗ್ರಾಹಕರನ್ನು ಆಕರ್ಷಿಸಲು ಹೆಚ್ಚುವರಿ ವಿವರಗಳನ್ನು ಸೇರಿಸಿ",
    ),
    "add_additional_email": MessageLookupByLibrary.simpleMessage(
      "ಹೆಚ್ಚುವರಿ ಇಮೇಲ್ ವಿಳಾಸ ಸೇರಿಸಿ",
    ),
    "add_more_photos": MessageLookupByLibrary.simpleMessage(
      "ಹೆಚ್ಚಿನ ಫೋಟೋಗಳನ್ನು ಸೇರಿಸಿ",
    ),
    "add_more_videos": MessageLookupByLibrary.simpleMessage(
      "ಹೆಚ್ಚಿನ ವೀಡಿಯೊಗಳನ್ನು ಸೇರಿಸಿ",
    ),
    "add_new": MessageLookupByLibrary.simpleMessage("ಹೊಸದು ಸೇರಿಸಿ"),
    "add_warehouse": MessageLookupByLibrary.simpleMessage("ಗೋದಾಮು ಸೇರಿಸಿ"),
    "add_warehouse_details": MessageLookupByLibrary.simpleMessage(
      "ಬಹುದೊಡ್ಡ ಗ್ರಾಹಕರನ್ನು ಆಕರ್ಷಿಸಲು ಗೋದಾಮು ವಿವರಗಳನ್ನು ಸೇರಿಸಿ",
    ),
    "add_warehouse_now": MessageLookupByLibrary.simpleMessage(
      "ಈಗ ಗೋದಾಮು ಸೇರಿಸಿ",
    ),
    "additional_phone_number": MessageLookupByLibrary.simpleMessage(
      "ಹೆಚ್ಚುವರಿ ಫೋನ್ ಸಂಖ್ಯೆ",
    ),
    "advanced_filters": MessageLookupByLibrary.simpleMessage(
      "ಆಧುನಿಕ ಶ್ರೇಣಿಗಳ ಫಿಲ್ಟರ್‌ಗಳು",
    ),
    "agricultural": MessageLookupByLibrary.simpleMessage("ಕೃಷಿ"),
    "all_topics": MessageLookupByLibrary.simpleMessage("ಎಲ್ಲಾ ವಿಷಯಗಳು"),
    "amenities": MessageLookupByLibrary.simpleMessage("ಸೌಕರ್ಯಗಳು"),
    "apply_filters": MessageLookupByLibrary.simpleMessage(
      "ಶ್ರೇಣಿಗಳನ್ನು ಅನ್ವಯಿಸಿ",
    ),
    "area_max_to_min": MessageLookupByLibrary.simpleMessage(
      "ಪ್ರದೇಶ (ಗರಿಷ್ಠದಿಂದ ಕನಿಷ್ಠಕ್ಕೆ)",
    ),
    "area_min_to_max": MessageLookupByLibrary.simpleMessage(
      "ಪ್ರದೇಶ (ಕನಿಷ್ಠದಿಂದ ಗರಿಷ್ಠಕ್ಕೆ)",
    ),
    "became_partner": MessageLookupByLibrary.simpleMessage("ಪಾರ್ಟ್ನರ್ ಆಗಿ"),
    "bids": MessageLookupByLibrary.simpleMessage("ಬಿಡ್ಸ್"),
    "bike_parking_slots": MessageLookupByLibrary.simpleMessage(
      "ಬೈಕ್ ಪಾರ್ಕಿಂಗ್ ಸ್ಲಾಟ್‌ಗಳು",
    ),
    "bts": MessageLookupByLibrary.simpleMessage("ಬಿಟಿಎಸ್"),
    "call_for_assistance": MessageLookupByLibrary.simpleMessage(
      "ಸಹಾಯಕ್ಕಾಗಿ ಕರೆ ಮಾಡಿ",
    ),
    "cancel": MessageLookupByLibrary.simpleMessage("ರದ್ದು ಮಾಡಿ"),
    "carpet_area": MessageLookupByLibrary.simpleMessage("ಕಾರ್ಪೆಟ್ ಪ್ರದೇಶ"),
    "centre_height": MessageLookupByLibrary.simpleMessage(
      "ಕೇಂದ್ರ ಎತ್ತರ (ಅಂಗುಚಿಯ)",
    ),
    "changes_saved_successfully": MessageLookupByLibrary.simpleMessage(
      "ಬದಲಾವಣೆಗಳನ್ನು ಯಶಸ್ವಿಯಾಗಿ ಉಳಿಸಲಾಗಿದೆ",
    ),
    "choose_from_camera": MessageLookupByLibrary.simpleMessage(
      "ಕ್ಯಾಮೆರಾದಿಂದ ಆಯ್ಕೆಮಾಡಿ",
    ),
    "choose_from_gallery": MessageLookupByLibrary.simpleMessage(
      "ಗ್ಯಾಲರಿಯಿಂದ ಆಯ್ಕೆಮಾಡಿ",
    ),
    "city": MessageLookupByLibrary.simpleMessage("ನಗರ"),
    "clear_all": MessageLookupByLibrary.simpleMessage("ಎಲ್ಲವನ್ನು ತೆರವುಗೊಳಿಸಿ"),
    "close": MessageLookupByLibrary.simpleMessage("ಬಂದ್ ಮಾಡಿ"),
    "cold_storage": MessageLookupByLibrary.simpleMessage("ತಂಪು ಗೋದಾಮು"),
    "coming_soon": MessageLookupByLibrary.simpleMessage("ಶೀಘ್ರದಲ್ಲೇ ಬರಲಿದೆ"),
    "company_name": MessageLookupByLibrary.simpleMessage("ಕಂಪನಿಯ ಹೆಸರು"),
    "complete_kyc": MessageLookupByLibrary.simpleMessage("ಕೆವೈಸಿ ಪೂರ್ಣಗೊಳಿಸಿ"),
    "connect_with_whatsapp": MessageLookupByLibrary.simpleMessage(
      "ವಾಟ್ಸಾಪ್ ಜೊತೆ ಸಂಪರ್ಕಿಸಿ",
    ),
    "construction_age_in_months": MessageLookupByLibrary.simpleMessage(
      "ನಿರ್ಮಾಣ ವಯಸ್ಸು (ಮಾಸಗಳಲ್ಲಿ)",
    ),
    "construction_type": MessageLookupByLibrary.simpleMessage("ನಿರ್ಮಾಣ ಪ್ರಕಾರ"),
    "construction_types": MessageLookupByLibrary.simpleMessage(
      "ನಿರ್ಮಾಣ ಪ್ರಕಾರಗಳು",
    ),
    "contracts": MessageLookupByLibrary.simpleMessage("ಒಪ್ಪಂದಗಳು"),
    "contracts_documents": MessageLookupByLibrary.simpleMessage(
      "ಒಪ್ಪಂದದ ದಾಖಲೆಗಳು",
    ),
    "dark_store": MessageLookupByLibrary.simpleMessage("ಡಾರ್ಕ್ ಸ್ಟೋರ್"),
    "designation": MessageLookupByLibrary.simpleMessage("ಹುದ್ದೆ"),
    "docks_height": MessageLookupByLibrary.simpleMessage(
      "ಡಾಕ್‌ಗಳ ಎತ್ತರ (ಅಂಗುಚಿಯ)",
    ),
    "done": MessageLookupByLibrary.simpleMessage("ಮುಗಿದಿದೆ"),
    "edit_profile": MessageLookupByLibrary.simpleMessage("ಪ್ರೊಫೈಲ್ ಸಂಪಾದಿಸಿ"),
    "edit_warehouse_details": MessageLookupByLibrary.simpleMessage(
      "ಗೋದಾಮು ವಿವರಗಳನ್ನು ಸಂಪಾದಿಸಿ",
    ),
    "electricity_in_kva": MessageLookupByLibrary.simpleMessage(
      "ವಿದ್ಯುತ್ (KVA)",
    ),
    "email_notifications": MessageLookupByLibrary.simpleMessage(
      "ಇಮೇಲ್ ಅಧಿಸೂಚನೆಗಳು",
    ),
    "enter_address": MessageLookupByLibrary.simpleMessage("ವಿಳಾಸ ನಮೂದಿಸಿ"),
    "express_interest": MessageLookupByLibrary.simpleMessage(
      "ಆಸಕ್ತಿ ವ್ಯಕ್ತಪಡಿಸಿ",
    ),
    "express_interest_to_get_callback": MessageLookupByLibrary.simpleMessage(
      "ಕಾಲ್‌ಬ್ಯಾಕ್ ಪಡೆಯಲು ಆಸಕ್ತಿ ಸೂಚಿಸಿ.",
    ),
    "factory": MessageLookupByLibrary.simpleMessage("ಕಾರ್ಖಾನೆ"),
    "filter": MessageLookupByLibrary.simpleMessage("ಫಿಲ್ಟರ್ ಮಾಡಿ"),
    "fire_noc": MessageLookupByLibrary.simpleMessage(
      "ಅಗ್ನಿಶಾಮಕ NOC: ನಿಮ್ಮ ಗೋದಾಮು ಅಗ್ನಿ ಸುರಕ್ಷಿತವಿದೆಯೆ?",
    ),
    "flexi_model_interest": MessageLookupByLibrary.simpleMessage(
      "ನೀವು ಫ್ಲೆಕ್ಸಿ ಮಾದರಿ ಆಸಕ್ತಿ ಹೊಂದಿದ್ದೀರಿ?",
    ),
    "flooring_type": MessageLookupByLibrary.simpleMessage("ಹೊರಗಿನ ಡೈಸಿ"),
    "follow_us_on": MessageLookupByLibrary.simpleMessage("ನಮಗೆ ಹಿಂಬಾಲಿಸಿ"),
    "furnishing_type": MessageLookupByLibrary.simpleMessage(
      "ಪರಿಕಾಲಿಕರ ಶ್ರೇಣಿಯಾ",
    ),
    "general": MessageLookupByLibrary.simpleMessage("ಸಾಮಾನ್ಯ"),
    "get_address": MessageLookupByLibrary.simpleMessage("ವಿಳಾಸ ಪಡೆಯಿರಿ"),
    "ground_floor": MessageLookupByLibrary.simpleMessage("ಭೂತಲ"),
    "have_clu_document": MessageLookupByLibrary.simpleMessage(
      "ನಿಮ್ಮ ಬಳಿ CLU ದಾಖಲಾತಿ ಇದೆಯೆ?",
    ),
    "hello": MessageLookupByLibrary.simpleMessage("ನಮಸ್ಕಾರ"),
    "help": MessageLookupByLibrary.simpleMessage("ಸಹಾಯ"),
    "help_and_support": MessageLookupByLibrary.simpleMessage(
      "ಸಹಾಯ ಮತ್ತು ಬೆಂಬಲ",
    ),
    "immediate": MessageLookupByLibrary.simpleMessage("ತಕ್ಷಣ"),
    "industrial_shed": MessageLookupByLibrary.simpleMessage("ಉದ್ಯಮ ಶೆಡ್"),
    "inner_length": MessageLookupByLibrary.simpleMessage(
      "ಆಂತರಿಕ дли_length (ಅಂಗುಚಿಯ)",
    ),
    "inner_width": MessageLookupByLibrary.simpleMessage("ಆಂತರಿಕ ಅಗಲ (ಅಂಗುಚಿಯ)"),
    "interested": MessageLookupByLibrary.simpleMessage("ಆಸಕ್ತಿಯು"),
    "is_base_available_for_rent": MessageLookupByLibrary.simpleMessage(
      "ಆಧಾರದ ಬಾಡಿಗೆಗೆ ಲಭ್ಯವಿದೆಯೆ?",
    ),
    "is_warehouse_available": MessageLookupByLibrary.simpleMessage(
      "ಗೋದಾಮು ಲಭ್ಯವಿದೆಯೆ?",
    ),
    "larger_collection_of_warehouses": MessageLookupByLibrary.simpleMessage(
      "ಹೆಚ್ಚು ಗೋದಾಮುಗಳನ್ನು ಸಂಗ್ರಹಿಸಿ",
    ),
    "location": MessageLookupByLibrary.simpleMessage("ಸ್ಥಳ"),
    "lock_in_period": MessageLookupByLibrary.simpleMessage("ಲಾಕ್ ಇನ್ ಅವಧಿ"),
    "log_out": MessageLookupByLibrary.simpleMessage("ಲಾಗ್ ಔಟ್"),
    "logout_confirmation": MessageLookupByLibrary.simpleMessage(
      "ನೀವು ಲಾಗ್‌ಔಟ್ ಮಾಡಲು ಖಚಿತವೇ?",
    ),
    "looking_for_rent_your_properties": MessageLookupByLibrary.simpleMessage(
      "ನೀವು ನಿಮ್ಮ ಆಸ್ತಿಗಳನ್ನು ಬಾಡಿಗೆಗೆ ಹುಡುಕುತ್ತಿದ್ದೀರಾ?",
    ),
    "maintenance_cost_per_sqft": MessageLookupByLibrary.simpleMessage(
      "ನಮೂನಾ ಖರ್ಚು (ಪ್ರತಿ ಚದರ ಅಡಿ)",
    ),
    "manage_photos": MessageLookupByLibrary.simpleMessage(
      "ಫೋಟೋಗಳನ್ನು ನಿರ್ವಹಿಸಿ",
    ),
    "manage_videos": MessageLookupByLibrary.simpleMessage(
      "ವೀಡಿಯೊಗಳನ್ನು ನಿರ್ವಹಿಸಿ",
    ),
    "manage_your_warehouse": MessageLookupByLibrary.simpleMessage(
      "ನಿಮ್ಮ ಗೋದಾಮನ್ನು ಶೀಘ್ರದಲ್ಲಿ ನಿರ್ವಹಿಸಿ!",
    ),
    "manpower": MessageLookupByLibrary.simpleMessage("ಮಾನವಶಕ್ತಿ"),
    "messenger": MessageLookupByLibrary.simpleMessage("ಮೆಸೇಂಜರ್"),
    "more": MessageLookupByLibrary.simpleMessage("ಇನ್ನಷ್ಟು"),
    "multi_storey_building": MessageLookupByLibrary.simpleMessage(
      "ಬಹುಮಹಡಿ ಕಟ್ಟಡ",
    ),
    "my_account": MessageLookupByLibrary.simpleMessage("ನನ್ನ ಖಾತೆ"),
    "my_profile": MessageLookupByLibrary.simpleMessage("ನನ್ನ ಪ್ರೊಫೈಲ್"),
    "name": MessageLookupByLibrary.simpleMessage("ಹೆಸರು"),
    "near_by": MessageLookupByLibrary.simpleMessage("ಹತ್ತಿರದಲ್ಲಿ"),
    "need_assistance": MessageLookupByLibrary.simpleMessage("ಸಹಾಯ ಬೇಕು"),
    "no": MessageLookupByLibrary.simpleMessage("ಇಲ್ಲ"),
    "no_interested_warehouse_found": MessageLookupByLibrary.simpleMessage(
      "ಯಾವುದೇ ಆಸಕ್ತ ಗೋದಾಮುಗಳನ್ನು ಕಂಡುಬರಲಿಲ್ಲ",
    ),
    "no_notifications": MessageLookupByLibrary.simpleMessage(
      "ಯಾವುದೇ ಅಧಿಕಾರನೋಡು ಇಲ್ಲ",
    ),
    "no_shortlisted_warehouses_found": MessageLookupByLibrary.simpleMessage(
      "ಯಾವುದೇ ಶಾರ್ಟ್‌ಲಿಸ್ಟ್ ಮಾಡಿದ ಗೋದಾಮುಗಳು ಕಂಡುಬರಲಿಲ್ಲ",
    ),
    "notification_setting": MessageLookupByLibrary.simpleMessage(
      "ಅಧಿಸೂಚನೆ ಸೆಟ್ಟಿಂಗ್",
    ),
    "notification_settings": MessageLookupByLibrary.simpleMessage(
      "ಅಧಿಸೂಚನೆ ಸೆಟ್ಟಿಂಗ್ಗಳು",
    ),
    "notifications": MessageLookupByLibrary.simpleMessage("ಅಧಿಕಾರನೋಡು"),
    "num_of_docks": MessageLookupByLibrary.simpleMessage("ಡಾಕ್‌ಗಳ ಸಂಖ್ಯೆ"),
    "num_of_fans": MessageLookupByLibrary.simpleMessage("ಪंखೆಗಳ ಸಂಖ್ಯೆ"),
    "num_of_floors_including_ground_floors":
        MessageLookupByLibrary.simpleMessage(
          "ಭೂತಲವನ್ನು ಸೇರಿಸಿ ಒಟ್ಟು ಮಹಡಿಯ ಸಂಖ್ಯೆಯನ್ನು",
        ),
    "num_of_lights": MessageLookupByLibrary.simpleMessage("ಬತ್ತಿಯ ಸಂಖ್ಯೆ"),
    "num_of_toilets": MessageLookupByLibrary.simpleMessage(
      "ಟಾಯ್ಲೆಟ್‌ಗಳ ಸಂಖ್ಯೆ",
    ),
    "open": MessageLookupByLibrary.simpleMessage("ತುಂಬಿ"),
    "open_space": MessageLookupByLibrary.simpleMessage("ತೆರೆದ ಸ್ಥಳ"),
    "others": MessageLookupByLibrary.simpleMessage("ಇತರೆ"),
    "owner_pan_card": MessageLookupByLibrary.simpleMessage("ಮಾಲಕನ PAN ಕಾರ್ಡ್"),
    "parking_land": MessageLookupByLibrary.simpleMessage("ಪಾರ್ಕಿಂಗ್ ಭೂಮಿ"),
    "payments": MessageLookupByLibrary.simpleMessage("ಪಾವತಿಗಳು"),
    "peb": MessageLookupByLibrary.simpleMessage("ಪಿಇಬಿ"),
    "phone_notifications": MessageLookupByLibrary.simpleMessage(
      "ಫೋನ್ ಅಧಿಸೂಚನೆಗಳು",
    ),
    "phone_number": MessageLookupByLibrary.simpleMessage("ಫೋನ್ ಸಂಖ್ಯೆ"),
    "please_upload_document": MessageLookupByLibrary.simpleMessage(
      "ದಯವಿಟ್ಟು ನಿಮ್ಮ ಡಾಕ್ಯುಮೆಂಟ್ ಅಪ್‌ಲೋಡ್ ಮಾಡಿ",
    ),
    "please_wait_for_sometime": MessageLookupByLibrary.simpleMessage(
      "ದಯವಿಟ್ಟು ಸ್ವಲ್ಪ ಸಮಯ ಕಾಯಿರಿ...",
    ),
    "post_your_property_free": MessageLookupByLibrary.simpleMessage(
      "ನಿಮ್ಮ ಆಸ್ತಿ ಉಚಿತವಾಗಿ ಪೋಸ್ಟ್ ಮಾಡಿ",
    ),
    "price_max_to_min": MessageLookupByLibrary.simpleMessage(
      "ಬೆಲೆ (ಗರಿಷ್ಠದಿಂದ ಕನಿಷ್ಠಕ್ಕೆ)",
    ),
    "price_min_to_max": MessageLookupByLibrary.simpleMessage(
      "ಬೆಲೆ (ಕನಿಷ್ಠದಿಂದ ಗರಿಷ್ಠಕ್ಕೆ)",
    ),
    "privacy_policy": MessageLookupByLibrary.simpleMessage("ಗೌಪ್ಯತಾ ನೀತಿ"),
    "provide_office_space": MessageLookupByLibrary.simpleMessage(
      "ನೀವು ಕಚೇರಿ ಜಾಗವನ್ನು ಒದಗಿಸಬಹುದೇ?",
    ),
    "push_notifications": MessageLookupByLibrary.simpleMessage(
      "ಪುಷ್ ಅಧಿಸೂಚನೆಗಳು",
    ),
    "radius_in_km": MessageLookupByLibrary.simpleMessage("ವಿಸ್ತೀರ್ಣ (ಕಿಮೀ)"),
    "rate_your_experience": MessageLookupByLibrary.simpleMessage(
      "ನಿಮ್ಮ ಅನುಭವವನ್ನು ಮೌಲ್ಯಮಾಪನ ಮಾಡಿ",
    ),
    "rcc": MessageLookupByLibrary.simpleMessage("ಆರ್‌ಸಿಸಿ"),
    "referral": MessageLookupByLibrary.simpleMessage("ರಿಫೆರಲ್"),
    "rent_per_sqft": MessageLookupByLibrary.simpleMessage(
      "ಪ್ರತಿ ಚದರ ಅಡಿ ಬಾಡಿಗೆ",
    ),
    "rent_range": MessageLookupByLibrary.simpleMessage("ಭಾಡಾ ಶ್ರೇಣಿ"),
    "save": MessageLookupByLibrary.simpleMessage("ಉಳಿಸಿ"),
    "seall": MessageLookupByLibrary.simpleMessage("ಎಲ್ಲವನ್ನೂ ನೋಡಿ..."),
    "search_by_location": MessageLookupByLibrary.simpleMessage(
      "ಸ್ಥಳದ ಪ್ರಕಾರ ಹುಡುಕಿ",
    ),
    "security_deposit": MessageLookupByLibrary.simpleMessage("ಭದ್ರತಾ ಠೇವಣಿ"),
    "select_date_of_possession": MessageLookupByLibrary.simpleMessage(
      "ಸ್ವಾಧೀನದ ದಿನಾಂಕ ಆಯ್ಕೆಮಾಡಿ",
    ),
    "select_location": MessageLookupByLibrary.simpleMessage(
      "ಸ್ಥಳವನ್ನು ಆಯ್ಕೆಮಾಡಿ",
    ),
    "select_rent_range": MessageLookupByLibrary.simpleMessage(
      "ಭಾಡಾ ಶ್ರೇಣಿಯನ್ನು ಆಯ್ಕೆಮಾಡಿ",
    ),
    "selfie_of_owner": MessageLookupByLibrary.simpleMessage("ಮಾಲಕನ ಸೆಲ್ಫಿ"),
    "shed": MessageLookupByLibrary.simpleMessage("ಶೆಡ್"),
    "shortlisted": MessageLookupByLibrary.simpleMessage(
      "ಶಾರ್ಟ್‌ಲಿಸ್ಟ್ ಮಾಡಿದವು",
    ),
    "side_height": MessageLookupByLibrary.simpleMessage("ಬದಿಯ ಎತ್ತರ (ಅಂಗುಚಿಯ)"),
    "skip_for_now": MessageLookupByLibrary.simpleMessage(
      "ಈಗ ಬದ್ಧಗೊಳ್ಳುವುದು ತಪ್ಪಿಸಿ",
    ),
    "sort": MessageLookupByLibrary.simpleMessage("ವರ್ಗೀಕರಿಸಿ"),
    "start_adding_warehouse": MessageLookupByLibrary.simpleMessage(
      "ನಿಮ್ಮ ಮೊದಲ ಗೋದಾಮು ಸೇರಿಸಲು ಪ್ರಾರಂಭಿಸಿ",
    ),
    "stay_tuned": MessageLookupByLibrary.simpleMessage(
      "ನಿಮಗಾಗಿ ಅದ್ಭುತ ಬರುವುದಕ್ಕೆ ಕಾಯಿರಿ!",
    ),
    "submit": MessageLookupByLibrary.simpleMessage("ಅರ್ಜಿಯು"),
    "submit_feedback": MessageLookupByLibrary.simpleMessage(
      "ಪ್ರತಿಕ್ರಿಯೆ ಸಲ್ಲಿಸಿ",
    ),
    "subscriptions": MessageLookupByLibrary.simpleMessage("ಚಂದಾದಾರಿತ್ವ"),
    "terms_and_conditions": MessageLookupByLibrary.simpleMessage(
      "ನಿಯಮಗಳು ಮತ್ತು ಶರತ್ತುಗಳು",
    ),
    "token_advance": MessageLookupByLibrary.simpleMessage("ಟೋಕನ್ ಮುಂದುವರಿದ"),
    "total_area": MessageLookupByLibrary.simpleMessage("ಒಟ್ಟು ವಿಸ್ತೀರ್ಣ"),
    "transportation": MessageLookupByLibrary.simpleMessage("ಸಾಗಾಣಿಕೆ"),
    "troubleshooting": MessageLookupByLibrary.simpleMessage("ಸಮಸ್ಯೆ ನಿವಾರಣೆ"),
    "truck_parking_slots": MessageLookupByLibrary.simpleMessage(
      "ಟ್ರಕ್ ಪಾರ್ಕಿಂಗ್ ಸ್ಲಾಟ್‌ಗಳು",
    ),
    "update_and_next": MessageLookupByLibrary.simpleMessage(
      "ನವೀಕರಿಸಿ ಮತ್ತು ಮುಂದುವರಿಸು",
    ),
    "update_and_proceed": MessageLookupByLibrary.simpleMessage(
      "ನವೀಕರಿಸಿ ಮತ್ತು ಮುಂದುವರಿಸಿ",
    ),
    "update_warehouse": MessageLookupByLibrary.simpleMessage(
      "ಗೋದಾಮನ್ನು ನವೀಕರಿಸಿ",
    ),
    "update_warehouse_address": MessageLookupByLibrary.simpleMessage(
      "ನಿಮ್ಮ ಗೋದಾಮಿನ ವಿಳಾಸವನ್ನು ನವೀಕರಿಸಿ",
    ),
    "update_warehouse_amenities": MessageLookupByLibrary.simpleMessage(
      "ಗೋದಾಮಿನ ಸೌಲಭ್ಯಗಳನ್ನು ನವೀಕರಿಸಿ",
    ),
    "update_warehouse_info": MessageLookupByLibrary.simpleMessage(
      "ನಿಮ್ಮ ಗೋದಾಮಿನ ಮಾಹಿತಿ ನವೀಕರಿಸಿ!",
    ),
    "upload": MessageLookupByLibrary.simpleMessage("ಅಪ್‌ಲೋಡ್ ಮಾಡಿ"),
    "upload_aadhar_back": MessageLookupByLibrary.simpleMessage(
      "ಆಧಾರ್ ಹಿಂಭಾಗ ಅಪ್‌ಲೋಡ್ ಮಾಡಿ",
    ),
    "upload_aadhar_front": MessageLookupByLibrary.simpleMessage(
      "ಆಧಾರ್ ಮುಂಭಾಗ ಅಪ್‌ಲೋಡ್ ಮಾಡಿ",
    ),
    "upload_document": MessageLookupByLibrary.simpleMessage(
      "ಡಾಕ್ಯುಮೆಂಟ್ ಅಪ್‌ಲೋಡ್ ಮಾಡಿ",
    ),
    "upload_media": MessageLookupByLibrary.simpleMessage("ಮೀಡಿಯಾ ಅಪ್ಲೋಡ್ ಮಾಡಿ"),
    "uploaded_successfully": MessageLookupByLibrary.simpleMessage(
      "ಯಶಸ್ವಿಯಾಗಿ ಅಪ್‌ಲೋಡ್ ಮಾಡಲಾಗಿದೆ",
    ),
    "verified_property_and_owner": MessageLookupByLibrary.simpleMessage(
      "ಪರಿಶೀಲಿತ ಆಸ್ತಿ ಮತ್ತು ಮಾಲೀಕ",
    ),
    "view_less": MessageLookupByLibrary.simpleMessage("ಕಡಿಮೆ ವೀಕ್ಷಿಸಿ"),
    "view_more": MessageLookupByLibrary.simpleMessage("ಇನ್ನಷ್ಟು ವೀಕ್ಷಿಸಿ"),
    "view_request": MessageLookupByLibrary.simpleMessage("ಅನುರೋಧವನ್ನು ನೋಡಿ"),
    "warehouse_dock_levelers": MessageLookupByLibrary.simpleMessage(
      "ನಿಮ್ಮ ಗೋದಾಮಿನಲ್ಲಿ ಡಾಕ್ ಲೆವಲರ್ಸ್ ಇದೆಯೆ?",
    ),
    "warehouse_images": MessageLookupByLibrary.simpleMessage(
      "ಗೋದಾಮಿನ ಚಿತ್ರಗಳು",
    ),
    "warehouse_name_owner_name": MessageLookupByLibrary.simpleMessage(
      "ಗೋದಾಮಿನ ಹೆಸರು / ಮಾಲಕರ ಹೆಸರು",
    ),
    "warehouse_near_you": MessageLookupByLibrary.simpleMessage(
      "ನಿಮ್ಮ ಹತ್ತಿರದ ಗೋದಾಮು..",
    ),
    "warehouse_power_backup": MessageLookupByLibrary.simpleMessage(
      "ನಿಮ್ಮ ಗೋದಾಮಿನಲ್ಲಿ ವಿದ್ಯುತ್ ಬ್ಯಾಕಪ್ ಇದೆಯೆ?",
    ),
    "warehouse_type": MessageLookupByLibrary.simpleMessage("ಗೋದಾಮು ಪ್ರಕಾರ"),
    "warehouse_types": MessageLookupByLibrary.simpleMessage("ಗೋದಾಮು ಪ್ರಕಾರಗಳು"),
    "warehouse_updated": MessageLookupByLibrary.simpleMessage(
      "ವೇರ್‌ಹೌಸ್ ನವೀಕರಿಸಲಾಗಿದೆ",
    ),
    "warehouses_near_you": MessageLookupByLibrary.simpleMessage(
      "ನಿಮ್ಮ ಹತ್ತಿರವೇರ್‌ಹೌಸ್‌ಗಳು",
    ),
    "warehousing": MessageLookupByLibrary.simpleMessage("ವೇರ್ಹೌಸಿಂಗ್"),
    "we_are_happy_to_help": MessageLookupByLibrary.simpleMessage(
      "ನಾವು ನಿಮ್ಮಿಗೆ ಸಹಾಯ ಮಾಡಲು ಸಂತೋಷಿಸುತ್ತೇವೆ!",
    ),
    "we_value_your_feedback": MessageLookupByLibrary.simpleMessage(
      "ನಾವು ನಿಮ್ಮ ಪ್ರತಿಕ್ರಿಯೆಯನ್ನು ಮೌಲ್ಯಯುತ ಮಾಡುತ್ತೇವೆ",
    ),
    "welcome": MessageLookupByLibrary.simpleMessage(
      "ನಮ್ಮ ಆಪ್‌ಗೆ ನಿಮಗೆ ಸ್ವಾಗತ!",
    ),
    "whose_partner_question": MessageLookupByLibrary.simpleMessage(
      "ನೀವು ಯಾರ ಪಾಲುದಾರರಾಗಲು ಬಯಸುತ್ತೀರಿ?",
    ),
    "within_15_days": MessageLookupByLibrary.simpleMessage("15 ದಿನಗಳೊಳಗೆ"),
    "within_30_days": MessageLookupByLibrary.simpleMessage("30 ದಿನಗಳೊಳಗೆ"),
    "within_60_days": MessageLookupByLibrary.simpleMessage("60 ದಿನಗಳೊಳಗೆ"),
    "write_something_valuable": MessageLookupByLibrary.simpleMessage(
      "ಊರ್ನು ಲಿಖಿ... (ಅಥವಾ ಸೇವೆಯನ್ನು ಆಸ್ವಾದಿಸು!)",
    ),
    "yes": MessageLookupByLibrary.simpleMessage("ಹೌದು"),
    "your_comment": MessageLookupByLibrary.simpleMessage("ನಿಮ್ಮ ಕಾಮೆಂಟ್"),
    "your_comments_and_ratings_help_us_to_improve":
        MessageLookupByLibrary.simpleMessage(
          "ನಿಮ್ಮ ಕಾಮೆಂಟ್‌ಗಳು ಮತ್ತು ರೇಟಿಂಗ್‌ಗಳು ನಮಗೆ ಉತ್ತಮವಾಗಲು ಸಹಾಯಮಾಡುತ್ತವೆ",
        ),
  };
}
