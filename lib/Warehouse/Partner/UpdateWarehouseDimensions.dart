...
class UpdateWarehouseDimensions extends StatefulWidget {
  ...
  const UpdateWarehouseDimensions({super.key,
    ...
    required this.cluDocument, required this.Id, required this.warehouse,
  });
...
class _UpdateWarehouseDimensionsState extends State<UpdateWarehouseDimensions> {
  ...
  void _submitForm() async {
    ...
    _showProgressIndicator(); // Show loading indicator
  ...
  void _showSuccessPopup() {
    ...
    // Auto-dismiss after 2 seconds and navigate to HomeScreen
    Future.delayed(const Duration(seconds: 2), () {
      Navigator.of(context).pop(); // Dismiss the success popup
  ...
  Map<String, dynamic> _prepareBodyData() {
    return {
      "Electricity": widget.electricity ?? "Unknown", // Default to "Unknown" if null
      "Power_backup": widget.powerBackup.toLowerCase() == "yes", // Convert string to boolean
      "Office_space": widget.provideOffice.toLowerCase() == "yes", // Convert string to boolean
      "Dock_levelers": widget.dockLevelers.toLowerCase() == "yes", // Convert string to boolean
      "NumberOfToilets": widget.toilets, // Default to 0 if parsing fails
      "Truck_ParkingSlot": int.tryParse(widget.parkingSlots) ?? 0, // Default to 0 if parsing fails
      "Bike_ParkingSlot": int.tryParse(widget.bikeSlots) ?? 0, // Default to 0 if parsing fails
      "NumberOfFans": int.tryParse(widget.fans.toString()) ?? 0, // Default to 0 if parsing fails
      "NumberOfLights": int.tryParse(widget.lights.toString()) ?? 0, // Default to 0 if parsing fails
      "FireComplaints": widget.fireComplaint.toLowerCase() == "yes", // Convert string to boolean
      "NumberOfDocks": _noOfDocks.text.trim().isNotEmpty ? _noOfDocks.text.trim() : "0", // Ensure non-empty string
      "Length": _length.text.trim().isNotEmpty ? _length.text.trim() : "0", // Ensure non-empty string
      "Width": _width.text.trim().isNotEmpty ? _width.text.trim() : "0", // Ensure non-empty string
      "SideHeight": _sideHeight.text.trim().isNotEmpty ? _sideHeight.text.trim() : "0", // Ensure non-empty string
      "CenterHeight": _centreHeight.text.trim().isNotEmpty ? _centreHeight.text.trim() : "0", // Ensure non-empty string
      "DocksOfHeight": _dockHeight.text.trim().isNotEmpty ? _dockHeight.text.trim() : "0", // Ensure non-empty string
      "FlexiModel": flexingModel.toLowerCase() == "yes", // Convert string to boolean
      "CluDocument": widget.cluDocument.toLowerCase() == "yes", // Convert string to boolean
      "FlooringType": _selectedFlooringType ?? "Unknown", // Default to "Unknown" if null
      "FurnishingType": _selectedFurnishingType ?? "Unknown", // Default to "Unknown" if null
      "WhouseId": widget.warehouse.wHouseId, // Warehouse ID (ensure it’s initialized)
...
class AnimatedDialog extends StatelessWidget {
  ...
  const AnimatedDialog({Key? key, required this.message}) : super(key: key);
